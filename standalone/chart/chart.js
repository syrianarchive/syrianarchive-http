...
d3.json("https://api.syrianarchive.org/incidents").then(function(d) {
  units = d.units;
  units = units.filter((obj) => obj.incident_date !== '');
  drawChart();
});

  const drawChart = () => {
  // settting up the data in this.props.units to the chart.
  units.forEach((d) => {
